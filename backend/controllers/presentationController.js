...
const deletePresentation = async (req, res) => {
  try {
    ...
    // Try to delete Cloudinary images if they exist
    try {
      const { cloudinary } = require('../config/cloudinary');
      if (process.env.CLOUDINARY_CLOUD_NAME) {
        for (const slide of presentation.slides) {
          if (slide.imagePath && slide.imagePath.includes('cloudinary')) {
            // Extract public_id from URL
            const parts = slide.imagePath.split('/upload/');
            if (parts[1]) {
              const publicId = parts[1].replace(/\.\w+$/, '');
      ...
      console.log('Cloudinary cleanup skipped:', e.message);
    }

    // Clean up local files
    const fs = require('fs');
    const localDir = require('path').join(__dirname, '..', 'converted-slides', req.params.id);
    if (fs.existsSync(localDir)) {
      fs.rmSync(localDir, { recursive: true, force: true });
    }
    if (presentation.originalFile && fs.existsSync(presentation.originalFile)) {
      fs.unlinkSync(presentation.originalFile);
    }
